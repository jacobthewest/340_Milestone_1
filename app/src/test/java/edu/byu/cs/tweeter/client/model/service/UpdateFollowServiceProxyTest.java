...
//    private UpdateFollowResponse failureResponse;
    private UpdateFollowServiceProxy updateFollowServiceProxySpy;
...
//        Mockito.when(mockServerFacade.updateFollow(followUserWeAlreadyFollow)).thenReturn(failureResponse);
        ...
        // Create a UpdateFollowService instance and wrap it with a spy that will use the mock edu.byu.cs.server.edu.byu.cs.tweeter.server.edu.byu.cs.shared.edu.byu.cs.tweeter.shared.model.edu.byu.cs.tweeter.server.service
        updateFollowServiceProxySpy = Mockito.spy(new UpdateFollowServiceProxy());
        Mockito.when(updateFollowServiceProxySpy.getServerFacade()).thenReturn(mockServerFacade);
    }

    @Test
    public void testUpdateFollow_validRequest_validRequestFollow() throws IOException {
        UpdateFollowResponse response = updateFollowServiceProxySpy.updateFollow(validRequestFollow);
    ...
    public void testUpdateFollow_validRequest_validRequestUnFollow() throws IOException {
        UpdateFollowResponse response = updateFollowServiceProxySpy.updateFollow(validRequestUnFollow);
