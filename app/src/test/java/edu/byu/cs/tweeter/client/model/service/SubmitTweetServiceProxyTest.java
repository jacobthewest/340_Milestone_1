...
public class SubmitTweetServiceProxyTest {
    ...
    private SubmitTweetServiceProxy submitTweetServiceProxySpy;
    ...
    public void setup() {
        ...
        // Create a SubmitTweetService instance and wrap it with a spy that will use the mock edu.byu.cs.server.edu.byu.cs.tweeter.server.edu.byu.cs.shared.edu.byu.cs.tweeter.shared.model.edu.byu.cs.tweeter.server.service
        submitTweetServiceProxySpy = Mockito.spy(new SubmitTweetServiceProxy());
        Mockito.when(submitTweetServiceProxySpy.getServerFacade()).thenReturn(mockServerFacade);
    }

    @Test
    public void testSubmitTweet_validRequest_correctResponse() throws IOException {
        SubmitTweetResponse response = submitTweetServiceProxySpy.submitTweet(validRequest);
        Assertions.assertEquals(successResponse, response);
    }

    @Test
    public void testSubmitTweet_invalidRequest_userDoesNotMatchStatus() throws IOException {
        SubmitTweetResponse response = submitTweetServiceProxySpy.submitTweet(invalidRequest1);
        Assertions.assertEquals(failureResponse, response);
    }

    @Test
    public void testSubmitTweet_invalidRequest_userIsNull() throws IOException {
        SubmitTweetResponse response = submitTweetServiceProxySpy.submitTweet(invalidRequest2);
        Assertions.assertEquals(failureResponse, response);
    }

    @Test
    public void testSubmitTweet_invalidRequest_statusIsNull() throws IOException {
        SubmitTweetResponse response = submitTweetServiceProxySpy.submitTweet(invalidRequest3);
        Assertions.assertEquals(failureResponse, response);
    }
